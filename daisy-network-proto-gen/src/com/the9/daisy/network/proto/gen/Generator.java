...
public class Generator {
	...
	private void generatePdlMsgTypeJson() {
		File xmlFile = new File(outDir + File.separatorChar + "PdlMsgType.xml");
		JSON json = new XMLSerializer().readFromFile(xmlFile);
		Writer writer = null;
		try {
			writer = new FileWriter(outDir + File.separatorChar
					+ "PdlMsgType.json");
		} catch (IOException e) {
			logger.error("error ", e);
		}
		...
		logger.info("generate [PdlMsgType.json] finish");
	}
